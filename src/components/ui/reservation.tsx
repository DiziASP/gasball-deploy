...
export const Reservation = ({lapangan, user} : Props): JSX.Element => {
    ...
    useEffect(() => {
      ...
      const todaySlot = lapangan.datePlots.find(slot => slot.date.setHours(0, 0, 0, 0) === todayDate);
    ...
    const handleDayClick: DayClickEventHandler = (day, modifiers, e) => {
        if (modifiers.selected) {
          ...
          const selectedSlot = lapangan.datePlots.find(slot => slot.date.getTime() === day.getTime());
    ...
    return (
      <div className="grid gap-2 h-full mb-5 w-fit">
        ...
        <div className="overflow-y-scroll h-11/12 bg-white rounded-2xl shadow-xl no-scrollbar">
          <ScheduleDay key={selectedDate.toISOString().split('T')[0]} selectedDate={selectedDate.toISOString().split('T')[0]} price={lapangan.pricePerHour} statusArray={selectedStatusArray} totalHours={0} fieldID={lapangan.id} customerID={user.id} customerName={user.full_name} ></ScheduleDay>
        </div>
      </div>
    );
  }
